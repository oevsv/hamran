...
int OFDMframeAssemble(int PHYmode, int cycl_pref, int *r_frame_buffer, int *sampleRate, int *c_buffer_len) // Marek to complete code !!!
{
    string message = "OE1XTU WRAN at 52.8 MHz";

    int dataCarrier = 480;
    int subCarrier = 1024;
    int useful_symbols = 22; //default number of symbols for cycl_pref = 4
    sampleRate = 3328000;   //default sample rate for cycl_pref = 4
    ...
    float bits_per_symbol = 1;                          //default value for PHYmode = 1

    // initialize frame generator properties
    ofdmflexframegenprops_s fgprops;
    ofdmflexframegenprops_init_default(&fgprops);
    fgprops.check = LIQUID_CRC_NONE;
    fgprops.fec0 = LIQUID_FEC_NONE;                     //default value for PHYmode = 1
    fgprops.fec1 = LIQUID_FEC_NONE; 
    fgprops.mod_scheme = LIQUID_MODEM_PSK2;             //default value for PHYmode = 1
    ...
    c_buffer_len = subCarrier + cp_len;                     // length of complex buffer

    // buffers
    liquid_float_complex c_buffer[c_buffer_len];            // time-domain complex buffer
    unsigned char header[8];                                // header data
    unsigned char payload[payload_len];                     // payload data
    unsigned char p[subCarrier];                            // subcarrier allocation array (null/pilot/data)
    unsigned int r_buffer[2 * c_buffer_len];                // time-domain buffer holding IQ samples
...
void *sendBeacon(void *threadID)
{
    ...
    auto t2 = t1;
    int shared_r_buffer[(int)(35*5*1024/4)];
    int shared_sampleRate;
    int shared_c_buffer_len;
    ...
    while (txON)
    {
        if (chrono::high_resolution_clock::now() - t2 > chrono::seconds(60 * interval))
        {
            ...
            OFDMframeAssemble(DEFAULT_PHY_MODE, DEFAULT_CYCL_PREFIX, r_shared_buffer, &shared_sampleRate; &shared_c_buffer_len);
...
int startSDRTXStream(int *r_frame_buffer)
{
    ...
    int ret = LMS_SendStream(&streamId, r_frame_buffer, c_buffer_len, nullptr, 1000);
