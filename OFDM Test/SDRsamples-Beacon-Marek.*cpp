...
int OFDMframeAssemble(int PHYmode, int cycl_pref) // Marek to complete code !!!
{
...
void *sendBeacon(void *threadID)
{
    ...
    while (txON)
    {
        if (chrono::high_resolution_clock::now() - t2 > chrono::seconds(60 * interval))
        {
            ...
            OFDMframeAssemble(DEFAULT_PHY_MODE, DEFAULT_CYCL_PREFIX);
...
int startSDRTXStream() // Merged with ODFMframeAssemble
{
    // Initialize stream
    lms_stream_t streamId;                        // stream structure
    streamId.channel = 0;                         // channel number
