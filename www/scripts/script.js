...
function connectWebSocket(spectrum) {
    ...
    {
        ws = new WebSocket("wss://" + window.location.host.substring(0, window.location.host.indexOf(':')) + ":8080");
    } else
    {
        ws = new WebSocket("ws://" + window.location.host.substring(0, window.location.host.indexOf(':')) + ":8080");
    }
